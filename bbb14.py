...
# 거울 탐지 알고리즘 (v3.24 - Search Box Scaling Fix)
...
def process_mirror_detection(frame_id):
    ...
    for label in unique_labels:
        ...
        # 4. 확정된 z_axis를 기준으로 최종 좌표축과 회전 행렬 계산
        ...
        # 첫 번째 박스에 적용할 기본 너비 배율과, 각 박스가 추가될 때마다 더해질 배율을 분리합니다.
        base_width_scale = 1.5  # 첫 번째 박스의 너비를 1.5배로 설정
        width_scale_increment = 0.5  # 다음 박스는 여기서 0.5배 더 커짐
...
def main():
    ...
    rospy.loginfo("▶ 거울 탐지 및 복원 노드 시작 (v3.24 - Search Box Scaling Fix)")
    rospy.spin()
