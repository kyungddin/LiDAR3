...
# 거울 탐지 알고리즘 (v3.7 - Manually Aligned Bounding Box)
...
def publish_cube(center, extent, orientation_q, frame_id="ouster", marker_id=0):
    """
    탐지된 객체 위치에 Rviz 시각화를 위한 CUBE 마커와 TEXT 마커를 발행합니다.
    """
    # 큐브 마커
    marker = Marker()
    ...
    marker.color = ColorRGBA(0.0, 0.0, 1.0, 0.5)  # 반투명 파란색
    marker.lifetime = rospy.Duration(1)  # 1초 동안 유지
    ...
    text_marker.pose.position.z = center[2] + extent[2] / 2 + 0.1  # 큐브 위에 표시
    text_marker.pose.orientation.w = 1.0
    text_marker.scale.z = 0.2  # 텍스트 크기
    text_marker.color = ColorRGBA(0.0, 0.0, 1.0, 1.0)  # 불투명 파란색
...
def callback(msg):
    ...
    # 시각화를 위해 노이즈가 아닌 모든 포인트를 한 번에 발행
...
def main():
    ...
    rospy.loginfo("▶ 거울 탐지 노드 시작 (v3.7 - Aligned BBox)")
    rospy.spin()
