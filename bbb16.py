...
# 거울 탐지 알고리즘 (v3.28 - Householder Reflection)
...
# (이전과 동일한 전역 변수, o3d_to_pointcloud2, publish_cube 등 함수들)
# ... (생략) ...
# 전역 변수 초기화
marker_pub = None
cloud_pub = None
...
def process_mirror_detection(frame_id):
    ...
    if frames_since_detection < DETECTION_TTL and last_mirror_state is not None:
        ...
        base_width_scale = 1.8;
        width_scale_increment = 0.5

        for i in range(num_boxes):
            box_center = center + z_axis * (step_distance * i + (step_distance / 2.0))
        ...
        if reflected_indices:
            ...
            # 1. 반사에 사용할 법선 벡터(n)와 평면 위의 점(Q) 추출
            n = z_axis  # 스무딩된 법선 벡터 (단위 벡터)
            Q = center  # 스무딩된 중심점
            ...
            # 3. 포인트들을 평면의 원점으로 이동 -> 반사 -> 다시 원래 위치로 이동
...
def main():
    ...
    rospy.loginfo("▶ 거울 탐지 및 복원 노드 시작 (v3.28 - Householder Reflection)")
    rospy.spin()
