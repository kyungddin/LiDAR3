...
# 거울 탐지 알고리즘 (v3.47 - Baseline + Z-Correction Only)
...
def process_mirror_detection(frame_id):
    # ... (Unchanged logic up to restoration)
    ...
    if frames_since_detection < DETECTION_TTL and last_mirror_state is not None:
        ...
        if reflected_indices:
            ...
            if Z_CORRECTION_FACTOR != 0.0:
                ...
                # --- ▼▼▼ 이 로그 라인을 추가해주세요 ▼▼▼ ---
                rospy.loginfo(
                    f"Z-CORRECT :: Factor={Z_CORRECTION_FACTOR}, Slope={mirror_z_slope:.3f}, Correction={z_correction:.3f}")
...
def main():
    ...
    rospy.loginfo("▶ 거울 탐지 및 복원 노드 시작 (v3.47 - Baseline + Z-Correction Only)")
    rospy.spin()
