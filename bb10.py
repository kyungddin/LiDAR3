...
# 250809 final + reflection (v16, The Final Reflection, Full Code)
# 첫반사복원성공
...
def reflect_point_cloud_across_plane(pcd_virtual, plane_center, plane_normal):
    ...
    # <<< MODIFIED: 벡터 방향을 수정하여 올바른 반사를 수행 >>>
    # 가상 점에서 평면으로의 벡터를 계산
    vec_to_plane = plane_center - points_virtual
    # 평면 법선에 투영하여 거리 계산
    dist_to_plane = np.dot(vec_to_plane, plane_normal)
    # 실제 위치 = 가상 위치 + 2 * (가상 위치에서 평면까지의 벡터)
...
def publish_front_text(center, normal, frame_id):
    # 법선 벡터는 센서를 향하도록 보정되었으므로, 텍스트를 살짝 앞으로 표시하려면 빼줘야 함
...
def callback(msg):
    ...
    pcd_full = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(np.array(points)))
    ...
    # 평면 뒤에 있는 점들(가상 객체) 식별
    # 법선 벡터는 센서를 향하므로, d < 0 인 점들이 거울 뒤 가상 공간에 있는 점들임
    dists_from_plane = np.dot(all_points - last_front_face_center, last_front_face_normal)
    virtual_point_indices = np.where(dists_from_plane < -0.05)[0]  # 5cm 이상 떨어진 점만
