...
def normal_to_angles(normal):
    ...
    # Z축과 이루는 각도 (절댓값 이용해 법선 방향 상하 무관)
...
def callback(msg):
    ...
    if last_front_face_center is None:
        ...
        angle_z, azimuth = normal_to_angles(closest_face[2])
        rospy.loginfo(f"법선 벡터 각도 - Z축과 각도: {angle_z:.2f}°, XY 평면 방위각: {azimuth:.2f}°")
        return
...
def main():
    ...
    rospy.init_node("bounding_box_detect", anonymous=True)

    marker_pub = rospy.Publisher("/mirror_bbox", Marker, queue_size=5)
    cloud_pub = rospy.Publisher("/mirror_filtered", PointCloud2, queue_size=5)
